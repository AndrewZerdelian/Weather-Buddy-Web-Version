...
import R_Current_Weather from "./ReusableComponents/R_Current_Weather";
import { WeeklyRequestByCityNameContext } from "../Context/WeeklyRequestByCityName";
import R_Todays_Details from "./ReusableComponents/R_Todays_Details";
import R_Todays_HighLight from "./ReusableComponents/R_Todays_HighLight";
import R_Weekly_Forcast from "./ReusableComponents/R_Weekly_Forcast";
 ...
 *   {R_Days?.dayOfWeek ? (
              ...
              Weather_Icon={SavingDataInState?.data?.list[0]?.weather[0]?.icon}
              Weather_Condition={
            ...
            <div className="col-span-12 md:col-span-3 mt-8 md:mr-5 px-6 bg-[#1C1C1E] pt-5 rounded-2xl">
              ...
              <div className="grid items-center justify-center">
                {Rearranged_Weekly_Request_By_City_Name.map(
                  (Weekly_Weather_Data, index) => (
                    <button
                      onClick={() =>
                        getdaysdetailsByCityName(Weekly_Weather_Data?.dayOfWeek)
                      }
                      key={index}
                    >
                      <R_Weekly_Forcast
              ...
              <div className="col-span-12 md:col-span-9 mt-8 px-5 pt-5 rounded-2xl bg-[#1C1C1E]">
                ...
                <div className="grid md:grid-cols-8 grid-cols-1 pt-5 gap-5">
                  {R_Days?.entries?.slice(0, 8).map((Weekly, index) => {
                    //// THE WORKING ONE THAT RENDER THE DAYS
              ...
              <div className="bg-[#111013] h-vh grid grid-cols-12 text-white font-extrabold px-5 ">
                            ...
                            <div className="col-span-12 md:col-span-9 mt-8 px-5 pt-5 rounded-2xl bg-[#1C1C1E]">
                <h1 className="text-2xl">Weathers Details</h1>
                <h1>WHAT CAN I DOOOO ! BEFORE PRESSING ANY DAY DATES </h1>
                <div className="grid md:grid-cols-8 grid-cols-1 pt-5 gap-5">
                  {Rearranged_Weekly_Request_By_City_Name[0]?.entries?.map(
                    (Weekly, index) => {
                      //////THE ONE THAT SHOULD WORK ON DEFAULT////////??
                      const Formated_Time = new Date(
                        Weekly.dt_txt
                      ).toLocaleString("en-US", {
                        hour: "numeric",
  ...
  return (
    <div>
      {SearchByCityName ? (
        <div>
          <R_Current_Weather
            Main_Temp={SavingDataInState?.data?.list[0]?.main?.temp.toFixed(0)}
            State_Name={SavingDataInState?.data?.city?.name}
            Countries_Name={SavingDataInState?.data?.city?.country}
            Weather_Icon={SavingDataInState?.data?.list[0]?.weather[0]?.icon}
            Weather_Condition={
          ...
          {Rearranged_Weekly_Request_By_City_Name.map(
            (Weekly_Weather_Data, index) => (
              <span
                onClick={() => getdaysdetails(Weekly_Weather_Data?.dayOfWeek)}
                key={index}
              >
                <R_Weekly_Forcast
                  Day_Of_The_Week={Weekly_Weather_Data?.dayOfWeek}
...
export default function Today() {
  ...
  return (
    <div>
      {SearchByCityName ? (
        <span>
          {Rearranged_Weekly_Request_By_City_Name[0]?.dayOfWeek ? (
            <div className="bg-[#111013] h-vh grid grid-cols-12 text-white font-extrabold px-5 ">
              <R_Current_Weather
                Main_Temp={SavingDataInState?.data?.list[0]?.main?.temp.toFixed(
                  0
              ...
              <div className="col-span-12 md:col-span-9 mt-8 px-5 pt-5 rounded-2xl bg-[#1C1C1E]" > 
                <h1 className="text-2xl">Weathers Details</h1>
                <h1>THE WORKING ONE ? </h1>                                              
                <div className="grid md:grid-cols-8 grid-cols-1 pt-5 gap-5">
                  {R_Days?.entries?.slice(0, 8).map((Weekly, index) => { //// THE WORKING ONE THAT RENDER THE DAYS 
                    const Formated_Time = new Date(
                      Weekly.dt_txt
                    ).toLocaleString("en-US", {
                      hour: "numeric",
            ...
            <div className="bg-[#111013] h-vh grid grid-cols-12 text-white font-extrabold px-5 ">
              ...
              <div className="col-span-12 md:col-span-9 mt-8 px-5 pt-5 rounded-2xl bg-[#1C1C1E]">
                ...
                <h1>WHAT CAN I DOOOO ! BEFORE PRESSING ANY DAY DATES </h1>
                <div className="grid md:grid-cols-8 grid-cols-1 pt-5 gap-5">
                  {Rearranged_Weekly_Request_By_City_Name?.entries[0]?.slice(0, 8)?.map((Weekly, index) => {//////////////??
                      const Formated_Time = new Date(
                        Weekly.dt_txt
                      ).toLocaleString("en-US", {
                        hour: "numeric",
